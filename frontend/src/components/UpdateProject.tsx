...
export default function UpdateProject({ project }: { project: ProjectI }) {
    ...
    const handleSubmit = async (event: React.FormEvent) => {
        ...
        let answer: string;
        answer = prompt("Please enter the code to update this project") as string
        if (!answer) return
        if (answer !== project.random) {
            toast.error("This project cannot be updated as the code you provided is incorrect!",
                { position: "top-center" })
            return
        }
        updateProject({
    ...
    return (
        <div className='my-10 sm:my-0'>
            <div className='px-5'>
                <form className="relative  rounded-md" onSubmit={handleSubmit}>
